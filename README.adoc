...
:plugin-name: Subscriber Later Binding - NR Electric
:sectnums:
:sectlinks:
:toc:
...
:uri-plugin: https://github.com/danyill/oscd-subscriber-lb-nr
:uri-ci-bundle: https://danyill.github.io/oscd-subscriber-lb-nr/oscd-subscriber-lb-nr.js
:uri-material-icons: https://fonts.google.com/icons
:uri-openscd-core-plugin-demo: https://danyill.github.io/oscd-subscriber-lb-nr/index.deploy.html
:source-highlighter: highlight.js

== What is this?

This plugin is intended to allow a user to achieve an improvement with the later binding subscriber plugin for NR Electric devices:

* Simultaneous subscriptions of quality together with other items for GOOSE
...
. This plugin does only two things:

* If two adjacent `ExtRef` elements have the same intAddr except there is a `q` then if in the subscribing dataset the adjacent FCDA elements are the same except the second is a quality attribute, then the quality will be automatically subscribed/unsubscribed. For example:
+
[source,xml]
----
<ExtRef intAddr="1-A:MUGO/GOINGGIO1.SPCSO6.stVal" serviceType="GOOSE" desc="Bay/Bus Close - 3 Pole"/>
<ExtRef intAddr="1-A:MUGO/GOINGGIO1.SPCSO6.q" serviceType="GOOSE" desc="Bay/Bus Close - 3 Pole (Quality)"/>
----
...
For a local resource, begins without a forward slash, e.g. `plugins/oscd-subscriber-lb-nr/dist/oscd-subscriber-lb-nr.js`. 
...
<.> Whether a document must be loaded for this plugin to be available
<.> URL which can be a local or remote resource. 
For a local resource, begins with a forward slash, e.g. `plugins/oscd-subscriber-lb-nr/dist/oscd-subscriber-lb-nr.js`. 
